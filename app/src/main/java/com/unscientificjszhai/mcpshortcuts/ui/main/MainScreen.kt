 ...
 * 主屏幕的 Compose 实现。
 *
 * @param viewModel 主屏幕的 ViewModel。
 */
...
fun MainScreen(viewModel: MainActivityViewModel = viewModel()) {
    ...
    Scaffold(
        ...
        if (!aiEnabled && serversWithTools.isEmpty() && recentHistory.isEmpty() && pinnedTools.isEmpty()) {
            // 无任何数据且未开启 AI 时显示空状态
            Box(
                modifier = Modifier
                    .fillMaxSize()
 ...
 * 点击直接调用，长按删除。
 *
 * @param pinned 固定工具实体数据。
 * @param onCallClick 点击执行调用时的回调。
 * @param onDeleteClick 点击删除按钮时的回调。
 */
@Composable
fun PinnedToolItem(
 ...
 * 历史记录列表项。
 *
 * @param history 历史记录实体数据。
 * @param innerPadding Scaffold 的内边距。
 * @param onClick 点击查看详情时的回调。
 * @param onDeleteClick 点击删除按钮时的回调。
 */
@Composable
fun HistoryItem(
 ...
 * 服务器详情列表项。
 *
 * @param item 包含服务器信息、连接状态及其工具列表的数据包装。
 * @param onRetry 点击重连按钮时的回调。
 * @param onUpdateTools 点击更新工具列表按钮时的回调。
 * @param onToolClick 点击工具列表项时的回调。
 * @param onEditClick 点击编辑服务器信息时的回调。
 * @param onDeleteClick 点击删除服务器时的回调。
 */
 ...
 * 服务器连接状态指示组件。
 *
 * @param state 当前的 MCP 客户端连接状态。
 * @param onRetry 点击重连或错误图标时的回调。
 */
@Composable
